@testset "Gnuplot output" begin
    ...
    VoroPlusPlus.__cxxwrap_draw_gnuplot("./image_data/single_cell.gnu", vc[], 0.0, 0.0, 0.0)
    ...
    VoroPlusPlus.__cxxwrap_draw_gnuplot("./image_data/single_cell_displaced.gnu", vc[], dx, dy, dz)
    cell_str = let buf = IOBuffer()
    ...
    VoroPlusPlus.draw_particles("./image_data/particles.gnu", con)
    VoroPlusPlus.draw_cells_gnuplot("./image_data/cells.gnu", con)
...
@testset "POV-Ray output" begin
    ...
    VoroPlusPlus.__cxxwrap_draw_pov("./image_data/single_cell.pov", vc[], 0.0, 0.0, 0.0)
    cell_str = let buf = IOBuffer()
        draw_pov(buf, vc)
        String(take!(buf))
    end
    ...
    VoroPlusPlus.__cxxwrap_draw_pov_mesh("./image_data/single_cell_mesh.pov", vc[], 0.0, 0.0, 0.0)
    cell_str = let buf = IOBuffer()
    ...
    VoroPlusPlus.__cxxwrap_draw_pov("./image_data/single_cell_displaced.pov", vc[], dx, dy, dz)
    cell_str = let buf = IOBuffer()
    ...
    VoroPlusPlus.__cxxwrap_draw_pov_mesh("./image_data/single_cell_displaced_mesh.pov", vc[], dx, dy, dz)
    cell_str = let buf = IOBuffer()
